...
function findClosestRoadWithinManhattanRadius(
    grid: Grid,
    originX: number,
    originY: number,
    targetX: number,
  ...
  for (const w of walkers) {
    ...
    if (isFood) {
      if (mx === undefined || my === undefined) {
        // invalid state -> freeze (no movement, no service)
        w.nextMoveAt = now + moveEveryMs;
        continue;
      }

      if (manhattan(w.x, w.y, mx, my) > MARKET_RADIUS) {
        const snapped =
          findClosestRoadWithinManhattanRadius(grid, mx, my, w.x, w.y, MARKET_RADIUS) ??
          findSpawnRoadNear(grid, mx, my);
    ...
    if (neighbors.length > 0) {
      ...
      if (w.kind === "water") {
        ...
        // MARKET RADIUS LIMIT (service) — always bound to the home market
        const filter = (hx: number, hy: number) => manhattan(hx, hy, mx!, my!) <= MARKET_RADIUS;
